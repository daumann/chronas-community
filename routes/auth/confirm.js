...
exports = module.exports = function(req, res) {
    ...
    logger.info("####################### logging in user",req.user)
	...
	var doSignIn = function() {
		...
		console.log('[auth.confirm] - Signing in user...' + locals.existingUser._id);
		console.log('------------------------------------------------------------');

		var userQuery = User.model.findById(locals.existingUser._id).select();
		userQuery.exec(function(err, myUser) {
				req.body.loginCount = myUser.loginCount+1;
				req.body.lastLogin = new Date().getTime();

				myUser.getUpdateHandler(req).process(req.body, { flashErrors: true, fields: 'loginCount,lastLogin' }, function(err) {

					logger.info("added counter for user!")
				});
		});
